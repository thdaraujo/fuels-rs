...
pub async fn launch_provider_and_get_wallet() -> LocalWallet {
    launch_custom_provider_and_get_wallet(None).await
}

/// Launches a provider with custom configuration and provides a test wallet
#[cfg(feature = "fuels-signers")]
pub async fn launch_custom_provider_and_get_wallet(addr: Option<SocketAddr>) -> LocalWallet {
    ...
    let (provider, _) = setup_test_provider(coins, addr).await;
...
pub async fn setup_test_provider(
    ...
    let (client, addr) = setup_test_client(coins, addr).await;
